...
import { getObjectWithFunctionCalling } from "./functionCalling";
import {
  ...
  await getState().setAsync(async (state) => {
    ...
    const narrate = async (action?: string) => {
      ...
      event.text = await backend.getNarration(narratePrompt(state, action), (token: string, count: number) => {
        streamingText += token;
        // Don't modify event.text during streaming - just accumulate in streamingText
        onToken(token, count);
        // Update the state to capture the streaming changes
        updateState();
      });
      // Ensure the final text is properly set
    ...
    try {
      ...
      if (state.view === "welcome") {
        ...
        const characters = await getObjectWithFunctionCalling(
        ...
        if (!(await getBoolean(checkIfSameLocationPrompt(state), onToken))) {
          ...
          step = ["Generating characters", "This typically takes between 30 seconds and 1 minute"];
          const characters = await getObjectWithFunctionCalling(generateCharactersPrompt, RawCharacter.array().length(5), onToken);
