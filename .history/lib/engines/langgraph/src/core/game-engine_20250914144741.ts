...
import { GameStateSchema, GameState } from "../types/game-state";
...
export class LangGraphGameEngine {
  ...
  private createGraph(): StateGraph<GameState> {
    // ⚠️ UNCONFIRMED: Using GameState interface directly instead of channels config
    // Based on LangGraph Python examples: StateGraph(State)
    const graph = new StateGraph<GameState>(GameState);
